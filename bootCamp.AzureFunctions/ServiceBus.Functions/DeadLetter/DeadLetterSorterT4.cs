...
{
    public static class DeadLetterSorterT4
    {
        private static readonly TelemetryClient _telemetry = new TelemetryClient();
        private const string functionName = "DeadLetterSorterT4";

        [FunctionName(functionName)]
        public static void Run([ServiceBusTrigger(QueueNames.DeadLetterSorterT4, AccessRights.Manage, Connection = "AzureWebJobsServiceBus")]string sbMessage, TraceWriter log)
        {
            LoggerHelper.WriteTrace(functionName, $"Atención se ha enviado a {QueueNames.DeadLetterSorterT4} | {sbMessage} | a las {DateTime.UtcNow.ToString("dd/MM/yyyy HH-mm-ss")}", log, TraceLevel.Info, _telemetry);
