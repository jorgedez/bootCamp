...
{
    public static class FlightSorterT4
    {
        private static TelemetryClient _telemetry = new TelemetryClient();
        private const string functionName = "FlightSorterT4";

        [FunctionName(functionName)]
        public static async Task Run([ServiceBusTrigger(QueueNames.Terminal4, AccessRights.Manage, Connection = "AzureWebJobsServiceBus")]string sbMessage, TraceWriter log)
        {
            try
            {
                var flight = JsonConvert.DeserializeObject<Flight>(sbMessage);
                LoggerHelper.WriteTrace(functionName, $"{QueueNames.Terminal4}-Informa: Se procesa el vuelo {flight.IATA}{flight.Code} procedente de " +
